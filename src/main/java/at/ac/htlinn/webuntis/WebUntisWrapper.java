...
public class WebUntisWrapper {
    ...
    Session session;
    private final Map<Integer, String> roomNameById = new HashMap<>();
    ...
    public WebUntisWrapper() throws IOException {
        ...
        session = Session.login(user, password, server, school);

        for (Rooms.RoomObject room : session.getRooms()) {
            roomNameById.put(room.getId(), room.getName());
        }
        try {
            for (Teachers.TeacherObject teacher : session.getTeachers()) {
                cacheTeacherName(teacher);
            }
        } catch (Exception e) {
            // getTeachers() requires elevated rights; teacher names will show as IDs
        }
    }

    private void cacheTeacherName(Teachers.TeacherObject teacher) {
        ...
        if (teacherName != null) {
            teacherNameById.put(teacher.getId(), teacherName);
        }
    }

    private void cacheTeacherName(JSONObject teacherJson) {
        int teacherId = teacherJson.optInt("id", -1);
        String teacherName = firstNonBlank(
                teacherJson.optString("fullName", null),
                teacherJson.optString("longName", null),
                teacherJson.optString("name", null)
        );
        if (teacherId >= 0 && teacherName != null) {
            teacherNameById.put(teacherId, teacherName);
        }
    }
    ...
    public Teachers.TeacherObject getTeacherById(int id) throws IOException {
        String cachedName = teacherNameById.get(id);
        if (cachedName != null) {
            return new Teachers.TeacherObject(cachedName, true, id, cachedName, "", "");
        }
        for (Teachers.TeacherObject teacher : session.getTeachers()) {
            if (teacher.getId() == id) {
                cacheTeacherName(teacher);
    ...
    public List<Teachers.TeacherObject> getAllTeachersFromTimetable(int classId, LocalDate from, LocalDate to) throws IOException {
        ...
        for (int i = 0; i < result.length(); i++) {
            ...
            for (int j = 0; j < teachers.length(); j++) {
                JSONObject teacherJson = teachers.getJSONObject(j);
                cacheTeacherName(teacherJson);
                ids.add(teacherJson.getInt("id"));
            }
        }

        List<Teachers.TeacherObject> list = new ArrayList<>();
        for (int id : ids) {
            Teachers.TeacherObject t = getTeacherById(id);
    ...
    public void dumpTimeTable(int classId, LocalDate day) throws IOException {
        ...
        for (int i = 0; i < result.length(); i++) {
            ...
            if (rooms != null && rooms.length() > 0) {
                JSONObject ro = rooms.getJSONObject(0);
                int roomId = ro.optInt("id", -1);
                String roomName = ro.optString("name", null);
                if (roomName != null && !roomName.isEmpty()) {
                    roomNameById.put(roomId, roomName);
                }
                System.out.println("Room:    " + roomNameById.getOrDefault(roomId, "ID:" + roomId));
            }

            JSONArray teachers = period.optJSONArray("te");
            if (teachers != null && teachers.length() > 0) {
                JSONObject te = teachers.getJSONObject(0);
                int teacherId = te.optInt("id", -1);
                cacheTeacherName(te);
                System.out.println("Teacher: " + teacherNameById.getOrDefault(teacherId, "ID:" + teacherId));
            }
        }
    }
